...
config_file = glob.glob(f'{validation_dir}/../../../config.yml', recursive=True)
...
if "AMI" in PROTOCOL_TRAIN or "CHiME5" in PROTOCOL_TRAIN:
    labels = np.delete(labels,
                       [i for i, value in enumerate(labels)
                        if value in ['KCHI', 'CHI']])
labels = list(labels)
# Read treshold
...
for npy in npy_files:
    ...
    with open(output_file, 'w') as f:
        for start, end in zip(starts[0], ends[0]):
            start_sec = start * args.step
            duration_sec = (end - start) * args.step
